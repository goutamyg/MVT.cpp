...
MVT::MVT(const char *model_path) {   
    this->mnn_net = std::unique_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(model_path));
    ...
    this->preprocess = std::unique_ptr<MNN::CV::ImageProcess>(MNN::CV::ImageProcess::create(config));
...
const MVT_Output& MVT::track(const cv::Mat &img)
{
    ...
    // Preprocess.
    this->preprocess->convert((uint8_t*)x_patch.data, x->width(), x->height(), 0, x);
    this->preprocess->convert((uint8_t*)this->z_patch.data, z->width(), z->height(), 0, z);
    
    // Run.
    auto ret = this->mnn_net->runSession(this->session);

    // Get output.
    auto scores  = this->mnn_net->getSessionOutput(this->session, "reg");
    ...
    // Postprocess.
    MNN::Tensor scores_host(scores, MNN::Tensor::CAFFE);
    MNN::Tensor sizes_host(sizes, MNN::Tensor::CAFFE);
    MNN::Tensor offsets_host(offsets, MNN::Tensor::CAFFE);
...
void MVT::cal_bbox(MNN::Tensor &scores_tensor, MNN::Tensor &offsets_tensor, MNN::Tensor &sizes_tensor, BBox &pred_box, float &max_score, float resize_factor) {
